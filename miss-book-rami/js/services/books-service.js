...
// 	return (book['reviews']) ? book['reviews'] : [];
// }
...
function removeBookReview(bookid, reviewidx) {
	var book = getBookById(bookid);
...
function saveBook(book) {
	...
	console.log('Sevice is saving the book', book);
	return Promise.resolve(book);
}

function getNextBookId(bookId) {
	var bookIdx = books.findIndex(currBook => currBook.id === bookId);
	var nextBook = (bookIdx < books.length-1)? books[bookIdx+1] : books[0] 
	return Promise.resolve(nextBook.id)
}
